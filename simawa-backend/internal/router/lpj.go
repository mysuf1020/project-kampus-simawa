...
func RegisterLPJRoutes(r *gin.Engine, cfg *config.Env, h *handler.LPJHandler, rbac *service.RBACService) {
	...
	api.POST("/:lpj_id/approve", middleware.RequireRoles(rbac, model.RoleBEMAdmin, model.RoleDEMAAdmin), h.Approve)
	api.POST("/:lpj_id/revision", middleware.RequireRoles(rbac, model.RoleAdmin, model.RoleBEMAdmin, model.RoleDEMAAdmin), h.Revision)
