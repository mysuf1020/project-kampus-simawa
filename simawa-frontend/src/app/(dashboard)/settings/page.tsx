...
export default function SettingsPage() {
  ...
  return (
    <Page>
      <Page.Header breadcrumbs={[{ href: '/dashboard', children: 'Dashboard' }, { href: '/settings', children: 'Pengaturan' }]}>
          ...
          <Tabs defaultValue="users" className="space-y-6">
            ...
            <TabsContent value="system" className="space-y-6 animate-in fade-in-50 duration-300">
              <Card className="border-neutral-200 shadow-sm">
                ...
                <CardContent>
                  {isLoading && (
                    <div className="flex items-center justify-center py-6 text-sm text-neutral-500">Memeriksa kesehatan sistem...</div>
                  )}
                  {isError && (
                    <div className="rounded-lg bg-red-50 p-3 text-sm text-red-600 border border-red-100">Gagal memuat status sistem. Pastikan backend berjalan.</div>
                  )}
                  {!isLoading && !isError && data && (
                    <div className="space-y-4">
                      ...
                      <div className="grid gap-3 sm:grid-cols-3">
                        <ServiceStatus label="Database" status={data.db_ok} icon={Database} />
                        <ServiceStatus label="Redis Cache" status={data.redis_ok} icon={Server} />
                        <ServiceStatus label="Object Storage" status={data.minio_ok} icon={HardDrive} />
                      </div>
                      <p className="text-[10px] text-neutral-400 text-center">Uptime: {Math.floor(data.uptime_seconds)} seconds</p>
                    </div>
                  )}
                </CardContent>
              </Card>
...
function ServiceStatus({ label, status, icon: Icon }: { label: string; status: boolean; icon: any }) {
  return (
    <div className={`flex flex-col items-center justify-center gap-2 rounded-xl border p-3 text-center transition-all ${
      status 
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-[10px] font-medium opacity-80">{status ? 'Operational' : 'Down'}</p>
      </div>
    </div>
  )
}
